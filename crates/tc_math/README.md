...
> **Layers differ sharply: the lower number-theory layer is functionally complete and
> only lacks optimization; the EC layer is feature-incomplete and not yet constant-time.**
...
| `ec` **Fp / F2m point layer** | ❌ incomplete | deliberate affine-only skeleton |
...
**The EC Fp / F2m point layer** (`fp_point` / `f2m_point`) is a deliberate skeleton:
- Only **affine** coordinates are implemented; `twice` / `add` / `neg` `todo!()` on
  any non-affine coordinate system.
- **Projective coordinate systems** are not implemented (the `coordinate_system` enum
  exists, but point arithmetic does not yet support them).
- **WNAF / windowed scalar multiplication** is not done; scalar mul uses plain
  double-and-add.
- Several `FpFieldElement` / `F2mFieldElement` operations are deferred until the point
  layer actually needs them.
...
- **EC**: no **WNAF / windowed NAF** scalar multiplication.
