...
use super::{NOEKEON_KEY_BYTES, NoekeonError};
...
impl NoekeonParams {
    ...
    pub fn new(key: &[u8]) -> Result<Self, NoekeonError> {
        let key: &[u8; NOEKEON_KEY_BYTES] = key
            .try_into()
            .map_err(|_| NoekeonError::InvalidKeyLength(key.len()))?;
        Ok(Self { key: *key })
    }
...
mod tests {
    ...
    fn rejects_invalid_key_length() {
        assert!(matches!(
            ...
            Err(NoekeonError::InvalidKeyLength(15))
        ));
        assert!(matches!(
            NoekeonParams::new(&[0u8; 24]),
            Err(NoekeonError::InvalidKeyLength(24))
        ));
    }
