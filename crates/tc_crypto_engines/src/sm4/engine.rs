//! SM4 block-cipher engine, key schedule, and round functions.

use tc_crypto_core::BlockCipher;

use super::{SM4_BLOCK_BYTES, Sm4Error, Sm4Params};

/// The SM4 S-box.
#[rustfmt::skip]
const SBOX: [u8; 256] = [
    ,
];

/// The fixed parameter constants `FK`.
const FK: [u32; 4] = [0xa3b1_bac6, 0x56aa_3350, 0x677d_9197, 0xb270_22dc];

/// The fixed key-schedule constants `CK`.
#[rustfmt::skip]
const CK: [u32; 32] = [
    0x0007_0e15, 0x1c23_2a31, 0x383f_464d, 0x545b_6269,
    0x7077_7e85, 0x8c93_9aa1, 0xa8af_b6bd, 0xc4cb_d2d9,
    0xe0e7_eef5, 0xfc03_0a11, 0x181f_262d, 0x343b_4249,
    0x5057_5e65, 0x6c73_7a81, 0x888f_969d, 0xa4ab_b2b9,
    0xc0c7_ced5, 0xdce3_eaf1, 0xf8ff_060d, 0x141b_2229,
    0x3037_3e45, 0x4c53_5a61, 0x686f_767d, 0x848b_9299,
    0xa0a7_aeb5, 0xbcc3_cad1, 0xd8df_e6ed, 0xf4fb_0209,
    0x1017_1e25, 0x2c33_3a41, 0x484f_565d, 0x646b_7279,
];

/// SM4 with a 128-bit key and 128-bit block.
pub struct Sm4Engine {
    /// 32 個輪金鑰;init 前為 `None` = 未初始化(解密時以反序展開)。
    rk: Option<[u32; 32]>,
}

impl Sm4Engine {
    /// Creates an uninitialised engine.
    pub fn new() -> Self {
        Self { rk: None }
    }
}

impl Default for Sm4Engine {
    fn default() -> Self {
        Self::new()
    }
}

impl BlockCipher for Sm4Engine {
    type Params<'a> = Sm4Params;
    type Error = Sm4Error;

    fn algorithm_name(&self) -> &str {
        "SM4"
    }

    fn block_size(&self) -> usize {
        SM4_BLOCK_BYTES
    }

    fn init(&mut self, for_encryption: bool, params: &Self::Params<'_>) -> Result<(), Self::Error> {
        self.rk = Some(expand_key(for_encryption, params.key()));
        Ok(())
    }

    fn process_block(&mut self, input: &[u8], output: &mut [u8]) -> Result<usize, Self::Error> {
        let rk = self.rk.as_ref().ok_or(Sm4Error::NotInitialised)?;
        if input.len() < SM4_BLOCK_BYTES || output.len() < SM4_BLOCK_BYTES {
            return Err(Sm4Error::BufferTooShort);
        }

        let mut x = [
            read_word(input, 0),
            read_word(input, 4),
            read_word(input, 8),
            read_word(input, 12),
        ];
        for round in rk.chunks_exact(4) {
            x[0] ^= t(x[1] ^ x[2] ^ x[3] ^ round[0]);
            x[1] ^= t(x[2] ^ x[3] ^ x[0] ^ round[1]);
            x[2] ^= t(x[3] ^ x[0] ^ x[1] ^ round[2]);
            x[3] ^= t(x[0] ^ x[1] ^ x[2] ^ round[3]);
        }

        // 輸出為反序 X3, X2, X1, X0。
        write_word(output, 0, x[3]);
        write_word(output, 4, x[2]);
        write_word(output, 8, x[1]);
        write_word(output, 12, x[0]);
        Ok(SM4_BLOCK_BYTES)
    }
}

/// The nonlinear substitution `tau`: apply the S-box to each byte.
fn tau(a: u32) -> u32 {
    let b0 = u32::from(SBOX[(a >> 24) as usize]);
    let b1 = u32::from(SBOX[((a >> 16) & 0xff) as usize]);
    let b2 = u32::from(SBOX[((a >> 8) & 0xff) as usize]);
    let b3 = u32::from(SBOX[(a & 0xff) as usize]);
    (b0 << 24) | (b1 << 16) | (b2 << 8) | b3
}

/// The round linear transform `L`.
fn l(b: u32) -> u32 {
    b ^ b.rotate_left(2) ^ b.rotate_left(10) ^ b.rotate_left(18) ^ b.rotate_left(24)
}

/// The round mixer-substitution `T = L . tau`.
fn t(z: u32) -> u32 {
    l(tau(z))
}

/// The key-schedule linear transform `L'`.
fn l_ap(b: u32) -> u32 {
    b ^ b.rotate_left(13) ^ b.rotate_left(23)
}

/// The key-schedule mixer-substitution `T' = L' . tau`.
fn t_ap(z: u32) -> u32 {
    l_ap(tau(z))
}

/// Expands the key into 32 round keys; decryption reverses their order.
fn expand_key(for_encryption: bool, key: &[u8; 16]) -> [u32; 32] {
    let k = [
        read_word(key, 0) ^ FK[0],
        read_word(key, 4) ^ FK[1],
        read_word(key, 8) ^ FK[2],
        read_word(key, 12) ^ FK[3],
    ];
    let mut rk = [0u32; 32];

    // 先以正向產生前四個輪金鑰。
    rk[0] = k[0] ^ t_ap(k[1] ^ k[2] ^ k[3] ^ CK[0]);
    rk[1] = k[1] ^ t_ap(k[2] ^ k[3] ^ rk[0] ^ CK[1]);
    rk[2] = k[2] ^ t_ap(k[3] ^ rk[0] ^ rk[1] ^ CK[2]);
    rk[3] = k[3] ^ t_ap(rk[0] ^ rk[1] ^ rk[2] ^ CK[3]);
    for i in 4..32 {
        rk[i] = rk[i - 4] ^ t_ap(rk[i - 3] ^ rk[i - 2] ^ rk[i - 1] ^ CK[i]);
    }

    if for_encryption {
        rk
    } else {
        let mut drk = [0u32; 32];
        for (i, &word) in rk.iter().enumerate() {
            drk[31 - i] = word;
        }
        drk
    }
}

/// Reads a big-endian 32-bit word at byte offset `off`.
fn read_word(input: &[u8], off: usize) -> u32 {
    u32::from_be_bytes(input[off..off + 4].try_into().unwrap())
}

/// Writes a big-endian 32-bit word at byte offset `off`.
fn write_word(output: &mut [u8], off: usize, value: u32) {
    output[off..off + 4].copy_from_slice(&value.to_be_bytes());
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn accessors_and_pre_init_errors() {
        let mut engine = Sm4Engine::new();
        assert_eq!(engine.algorithm_name(), "SM4");
        assert_eq!(engine.block_size(), 16);
        assert_eq!(
            engine.process_block(&[0u8; 16], &mut [0u8; 16]),
            Err(Sm4Error::NotInitialised)
        );
    }

    #[test]
    fn short_buffers_are_rejected() {
        let params = Sm4Params::new(&[0u8; 16]).unwrap();
        let mut engine = Sm4Engine::new();
        engine.init(true, &params).unwrap();
        assert_eq!(
            engine.process_block(&[0u8; 15], &mut [0u8; 16]),
            Err(Sm4Error::BufferTooShort)
        );
        assert_eq!(
            engine.process_block(&[0u8; 16], &mut [0u8; 15]),
            Err(Sm4Error::BufferTooShort)
        );
    }
}
