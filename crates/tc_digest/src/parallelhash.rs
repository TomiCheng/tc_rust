...
//! the fixed digest, or `0` in XOF mode ([`Xof`](tc_crypto_core::Xof)) — so the two
...
use tc_crypto_core::{Digest, TryDigest, TryXof, Xof};
...
impl ParallelHash {
    ...
    pub fn with_output_size(bit_length: usize, s: &[u8], b: usize, output_size_bits: usize) -> Self {
...
mod tests {
    ...
    fn chunked_matches_whole() {
        ...
        let mut h = ParallelHash::new(128, b"Parallel Data", 12);
        for chunk in [&msg[..5], &msg[5..12], &msg[12..13], &msg[13..48], &msg[48..]] {
            h.update(chunk);
        }
        let mut out = vec![0u8; h.digest_size()];
        h.do_final(&mut out);
