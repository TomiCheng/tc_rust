...
mod tests {
    ...
    fn zero_pad_z() {
        ...
        let mut o = [0u8; 20];
        d.output(&mut o);
        assert_eq!(o.to_vec(), unhex("6e393540387004f087c4180db008acf6825190cf"));

        let mut d = CShakeDigest::new(128, b"", &[0u8; 329]);
        let mut o = [0u8; 20];
        d.output(&mut o);
        assert_eq!(o.to_vec(), unhex("309bd7c285fcf8b839c9686b2cc00bd578947bee"));

        let mut d = CShakeDigest::new(128, &[0u8; 29], &[0u8; 300]);
        let mut o = [0u8; 20];
        d.output(&mut o);
        assert_eq!(o.to_vec(), unhex("ff6aafd83b8d22fc3e2e9b9948b581967ed9c5e7"));
    }
