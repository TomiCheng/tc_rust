...
fn round(state: &mut [u64; 5], constant: u64) {
    ...
    let t0 = x0 ^ x1 ^ x2 ^ x3 ^ constant ^ (x1 & (x0 ^ x2 ^ x4 ^ constant));
    let t1 = x0 ^ x2 ^ x3 ^ x4 ^ constant ^ ((x1 ^ x2 ^ constant) & (x1 ^ x3));
    let t2 = x1 ^ x2 ^ x4 ^ constant ^ (x3 & x4);
    let t3 = x0 ^ x1 ^ x2 ^ constant ^ ((!x0) & (x3 ^ x4));
