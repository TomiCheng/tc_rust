...
use tc_block_cipher::rijndael::{RijndaelConfig, ValidRijndaelConfig};
...
fn run_vector<const BLOCK_COLUMNS: usize, const KEY_COLUMNS: usize>(
    key: &str,
    plaintext: &str,
    ciphertext: &str,
) where
    RijndaelConfig<BLOCK_COLUMNS, KEY_COLUMNS>: ValidRijndaelConfig<BLOCK_COLUMNS>,
{
    ...
    let bytes = BLOCK_COLUMNS * 4;
    let params = RijndaelParams::<KEY_COLUMNS>::new(&key).unwrap();
    let mut engine = RijndaelEngine::<BLOCK_COLUMNS, KEY_COLUMNS>::new();
    let mut output = vec![0u8; bytes];

    engine.init(CipherDirection::Encrypt, &params).unwrap();
    assert_eq!(engine.process_block(&plaintext, &mut output).unwrap(), bytes);
    assert_eq!(output, ciphertext, "encrypt {}-bit block", BLOCK_COLUMNS * 32);

    engine.init(CipherDirection::Decrypt, &params).unwrap();
    engine.process_block(&ciphertext, &mut output).unwrap();
    assert_eq!(output, plaintext, "decrypt {}-bit block", BLOCK_COLUMNS * 32);
}
...
fn run_monte_carlo<const KEY_COLUMNS: usize>(
    iterations: usize,
    key: &str,
    plaintext: &str,
    ciphertext: &str,
) where
    RijndaelConfig<4, KEY_COLUMNS>: ValidRijndaelConfig<4>,
{
    let key = unhex(key);
    let plaintext = unhex(plaintext);
    let ciphertext = unhex(ciphertext);
    let params = RijndaelParams::<KEY_COLUMNS>::new(&key).unwrap();
    let mut engine = RijndaelEngine::<4, KEY_COLUMNS>::new();
