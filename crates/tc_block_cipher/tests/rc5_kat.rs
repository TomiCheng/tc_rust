...
fn run_rc5_32<const ROUNDS: usize>(key: &str, iv: &str, plaintext: &str, ciphertext: &str) {
    run::<Rc532Engine<ROUNDS>>(8, key, iv, plaintext, ciphertext);
}

fn run_rc5_64<const ROUNDS: usize>(key: &str, iv: &str, plaintext: &str, ciphertext: &str) {
    run::<Rc564Engine<ROUNDS>>(16, key, iv, plaintext, ciphertext);
}

fn run<E>(block: usize, key: &str, iv: &str, plaintext: &str, ciphertext: &str)
where
    ...
    for<'a> E: BlockCipherInit<Params<'a> = Rc5Params>,
{
    ...
    let params = Rc5Params::new(&key).unwrap();
