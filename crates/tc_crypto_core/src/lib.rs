...
//!
//! Unlike `rand_core` (an external community trait we depend on), the digest
//! trait is ported here as **our own**, because every digest algorithm we port
//! comes from Bouncy Castle and mirrors its `IDigest` shape.
