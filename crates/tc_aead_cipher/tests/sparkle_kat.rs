#[cfg(feature = "alloc")]
use tc_aead_cipher::sparkle::OwnedParams;
use tc_aead_cipher::{
    AeadCipherError,
    sparkle::{BorrowedParams, Engine, Variant},
};
...
fn rejects_invalid_lengths_modified_tags_and_short_ciphertexts() {
    for variant in VARIANTS {
        let (key, nonce) = material(variant);
        let mut engine = Engine::new(variant);
        let params = BorrowedParams::new(&key[..key.len() - 1], &nonce);
        assert_eq!(
            engine.init(CipherDirection::Encrypt, &params),
            Err(AeadCipherError::InvalidKeyLength(key.len() - 1))
        );

        let params = BorrowedParams::new(&key, &nonce[..nonce.len() - 1]);
        assert_eq!(
            engine.init(CipherDirection::Encrypt, &params),
...
#[cfg(feature = "alloc")]
#[test]
fn owned_params_match_a_schwaemm192_192_vector() {
    let variant = Variant::Schwaemm192_192;
    let kat = kats(variant).last().unwrap();
    let (key, nonce) = material(variant);
    let plaintext = decode_hex(kat.plaintext);
    let aad = decode_hex(kat.aad);
