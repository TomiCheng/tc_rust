//! Tiger message digest, ported from Bouncy Castle's `TigerDigest`.
//!
//! Tiger processes 512-bit blocks with three 64-bit chaining words. Each block
//! runs three eight-round passes, separated by a message-word key schedule, and
//! uses four fixed 256-entry S-boxes. Unlike most digests in this crate, Tiger's
//! first padding byte is `0x01` (Tiger2 uses `0x80`).
...
/// The 192-bit Tiger message digest.
#[derive(Clone)]
pub struct TigerDigest {
    /// Tiger 的三個 64-bit chaining words。
    state: [u64; 3],
    /// 共用的 64-byte 區塊緩衝。
    buf: MdBuffer<64>,
}

impl Default for TigerDigest {
    fn default() -> Self {
        TigerDigest {
            state: IV,
            buf: MdBuffer::new(),
...
impl TigerDigest {
    /// Creates a fresh Tiger digest.
    pub fn new() -> Self {
        Self::default()
    }
}

/// Tiger 的單輪混合。呼叫端輪替 `a`、`b`、`c` 的傳入順序。
#[inline(always)]
fn round(a: &mut u64, b: &mut u64, c: &mut u64, x: u64, mul: u64) {
    ...
    *a = a.wrapping_sub(
        T1[(value & 0xff) as usize]
            ^ T2[((value >> 16) & 0xff) as usize]
            ^ T3[((value >> 32) & 0xff) as usize]
            ^ T4[((value >> 48) & 0xff) as usize],
    );
    *b = b
        .wrapping_add(
            T4[((value >> 8) & 0xff) as usize]
                ^ T3[((value >> 24) & 0xff) as usize]
                ^ T2[((value >> 40) & 0xff) as usize]
                ^ T1[((value >> 56) & 0xff) as usize],
        )
        .wrapping_mul(mul);
}

/// 八輪 pass;下一輪將三個 chaining words 循環換位。
#[inline(always)]
...
impl TryDigest for TigerDigest {
    ...
    fn try_do_final(&mut self, output: &mut [u8]) -> Result<usize, Self::Error> {
        {
            ...
            buf.finish_with_pad(0x01, &bit_length.to_le_bytes(), |block| {
