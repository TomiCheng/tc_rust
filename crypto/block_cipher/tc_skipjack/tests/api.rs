...
use tc_skipjack::{BLOCK_BYTES, KEY_BYTES, SkipjackEngine};

#[test]
fn writes_algorithm_name() {
    let mut name = String::new();
    let algorithm: &dyn AlgorithmName = &SkipjackEngine::new();
    algorithm.write_algo_name(&mut name).unwrap();
    assert_eq!(name, "SKIPJACK");
}
...
fn validates_state_key_length_and_buffers() {
    ...
    for length in [0, KEY_BYTES - 1, KEY_BYTES + 1, 16] {
