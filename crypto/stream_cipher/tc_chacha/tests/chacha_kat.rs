//! Original ChaCha vectors generated by the eSTREAM reference implementation
//! and used by Bouncy Castle's `ChaChaTest`.
...
#[test]
fn bc_chacha20_vector_and_offsets() {
    let key = unhex("80000000000000000000000000000000");
    let stream = keystream(20, &key, &[0u8; IV_BYTES], 512);
    assert_eq!(
        &stream[..64],
        unhex(
...
#[test]
fn bc_reduced_round_vectors() {
    let key = unhex("80000000000000000000000000000000");
    assert_eq!(
        keystream(12, &key, &[0u8; IV_BYTES], 64),
        unhex(
    ...
    assert_eq!(
        keystream(8, &key, &[0u8; IV_BYTES], 64),
        unhex(
...
#[test]
fn bc_256_bit_key_and_counter_boundary_vectors() {
    let key = unhex(
...
#[test]
fn reset_chunking_single_byte_and_directions_match() {
    let params = KeyWithIvRef::new(&[0x11; 32], &[0x22; IV_BYTES]);
