...
//! # 記憶體清除政策
//!
//! Owned 金鑰容器在 drop 時清除各欄位的有效位元組與 spare capacity；
//! Ref 容器則由呼叫端負責清除。
//! 堆引擎在 drop 或成功重新初始化時清除原狀態的整數欄位。固定寬度引擎保留
//! `Copy`，不會自動清除金鑰狀態。兩個 CRT 後端的盲化因子與模反元素使用區域守衛，
//! 但固定寬度值的按值副本不受守衛保護。
//! 清除涵蓋目前有效資料與堆配置的 spare capacity，不含舊配置、其他副本或其餘運算中間值；
//! 析構函式未執行時也不會觸發清除。這些政策不改變各後端的計時性質。
