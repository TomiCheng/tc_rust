...
pub(crate) fn div_rem(dividend: &[Limb], divisor: &[Limb]) -> (Vec<Limb>, Vec<Limb>) {
    ...
    for offset in (0..quotient_len).rev() {
        let numerator = ((normalized_dividend[offset + divisor_len].to_word() as WideWord) << Word::BITS)
        ...
        for index in 0..divisor_len {
            let product =
                estimate_word as WideWord * normalized_divisor[index].to_word() as WideWord + borrow;
            let (difference, underflow) = normalized_dividend[offset + index].to_word()
        ...
        let (difference, negative) = normalized_dividend[high_index].to_word()
        ...
        if negative {
            ...
            normalized_dividend[high_index] = Limb::new(normalized_dividend[high_index].to_word().wrapping_add(carry.to_word()));
        }
...
pub(crate) fn bit_len(words: &[Limb]) -> usize {
    ...
    if len == 0 {
        ...
        (len - 1) * Word::BITS as usize + (Word::BITS - words[len - 1].to_word().leading_zeros()) as usize
    }
}
...
pub(crate) fn fixed_mul_wide<const N: usize>(
    ...
    for (left_index, left) in lhs.iter().enumerate() {
        ...
        for (right_index, right) in rhs.iter().enumerate() {
            ...
            let wide =
                left.to_word() as WideWord * right.to_word() as WideWord + current as WideWord + carry as WideWord;
...
pub(crate) fn fixed_wide_rem<const N: usize>(
    ...
    for (index, word) in remainder.iter_mut().take(modulus_len).enumerate() {
        *word = Limb::new(fixed_normalized_wide_word(
            low,
            high,
    ...
    for input_index in (0..first_quotient_input).rev() {
        ...
        let numerator =
            ((window_high as WideWord) << Word::BITS) | remainder[modulus_len - 1].to_word() as WideWord;
        ...
        for index in 0..modulus_len {
            let product =
                estimate_word as WideWord * normalized_modulus[index].to_word() as WideWord + borrow;
            let (difference, underflow) = remainder[index].to_word().overflowing_sub(product as Word);
...
mod tests {
    ...
    fn fixed_division_sign_and_gcd_helpers_cover_all_results() {
        ...
        assert_eq!(
            fixed_gcd(&[Limb::new(48), Limb::new(0)], &[Limb::new(18), Limb::new(0)]),
    ...
    fn dynamic_normalization_and_comparison_are_directly_covered() {
        ...
        assert_eq!(cmp(&[Limb::new(1), Limb::new(0)], &[Limb::new(2)]), Ordering::Less);
    }
    ...
    fn optimized_square_matches_independent_multiplication_for_multi_limb_values() {
        let cases = [
            ...
            vec![Limb::new(Word::MAX), Limb::new(0), Limb::new(Word::MAX), Limb::new(17)],
        ];
    ...
    fn karatsuba_matches_schoolbook_around_the_threshold_and_for_uneven_lengths() {
        ...
        fn random_words(state: &mut u64, len: usize) -> Vec<Limb> {
            let mut words = (0..len).map(|_| Limb::new(next_word(state))).collect::<Vec<_>>();
    ...
    fn shifts_and_truncation_checks_cover_word_and_bit_boundaries() {
        ...
        assert_eq!(bit_len(&[Limb::new(0), Limb::new(8)]), Word::BITS as usize + 4);
        assert_eq!(bit_len(&[]), 0);
    }
