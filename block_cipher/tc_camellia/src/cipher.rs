//! Camellia T-tables, key schedule, and block transformation.
...
pub(super) struct CamelliaKeySchedule {
    subkey: [u32; 96],
    whitening_key: [u32; 8],
    fl_key: [u32; 12],
    key_is_128: bool,
...
impl CamelliaKeySchedule {
    /// Creates an empty schedule. Constant time.
    ...
    /// Expands a key. Variable time: secret-dependent S-box lookups.
    ...
    /// Transforms a block. Variable time: secret-dependent S-box lookups.
    pub(super) fn process_block(&self, input: &[u8; BLOCK_BYTES], output: &mut [u8; BLOCK_BYTES]) {
    ...
    pub(super) fn process_block_with(
        ...
        for (index, value) in [
            state[2] ^ self.whitening_key[4],
            state[3] ^ self.whitening_key[5],
            state[0] ^ self.whitening_key[6],
            state[1] ^ self.whitening_key[7],
        ]
        .into_iter()
        .enumerate()
        {
    ...
    fn set_key_128(&mut self, for_encryption: bool, k: &mut [u32; 8], ka: &mut [u32; 4]) {
        let mut temporary = [0u32; 4];

        if for_encryption {
            self.whitening_key[..4].copy_from_slice(&k[..4]);
            roldq(15, k, 0, &mut self.subkey, 4);
            roldq(30, k, 0, &mut self.subkey, 12);
            roldq(15, k, 0, &mut temporary, 0);
            self.subkey[18] = temporary[2];
            self.subkey[19] = temporary[3];
            roldq(17, k, 0, &mut self.fl_key, 4);
            roldq(17, k, 0, &mut self.subkey, 24);
            roldq(17, k, 0, &mut self.subkey, 32);

            self.subkey[..4].copy_from_slice(ka);
            roldq(15, ka, 0, &mut self.subkey, 8);
            roldq(15, ka, 0, &mut self.fl_key, 0);
            roldq(15, ka, 0, &mut temporary, 0);
            self.subkey[16] = temporary[0];
            self.subkey[17] = temporary[1];
            roldq(15, ka, 0, &mut self.subkey, 20);
            roldqo32(34, ka, 0, &mut self.subkey, 28);
            roldq(17, ka, 0, &mut self.whitening_key, 4);
        } else {
            self.whitening_key[4..].copy_from_slice(&k[..4]);
            decroldq(15, k, 0, &mut self.subkey, 28);
            decroldq(30, k, 0, &mut self.subkey, 20);
            decroldq(15, k, 0, &mut temporary, 0);
            self.subkey[16] = temporary[0];
            self.subkey[17] = temporary[1];
            decroldq(17, k, 0, &mut self.fl_key, 0);
            decroldq(17, k, 0, &mut self.subkey, 8);
            decroldq(17, k, 0, &mut self.subkey, 0);

            self.subkey[34] = ka[0];
            self.subkey[35] = ka[1];
            self.subkey[32] = ka[2];
            self.subkey[33] = ka[3];
            decroldq(15, ka, 0, &mut self.subkey, 24);
            decroldq(15, ka, 0, &mut self.fl_key, 4);
            decroldq(15, ka, 0, &mut temporary, 0);
            self.subkey[18] = temporary[2];
            self.subkey[19] = temporary[3];
            decroldq(15, ka, 0, &mut self.subkey, 12);
            decroldqo32(34, ka, 0, &mut self.subkey, 4);
            roldq(17, ka, 0, &mut self.whitening_key, 0);
        }
        temporary.zeroize();
    }

    fn set_key_192_or_256(
        &mut self,
        for_encryption: bool,
        k: &mut [u32; 8],
        ka: &mut [u32; 4],
        ...
        if for_encryption {
            self.whitening_key[..4].copy_from_slice(&k[..4]);
            roldqo32(45, k, 0, &mut self.subkey, 16);
            roldq(15, k, 0, &mut self.fl_key, 4);
            roldq(17, k, 0, &mut self.subkey, 32);
            roldqo32(34, k, 0, &mut self.subkey, 44);

            roldq(15, k, 4, &mut self.subkey, 4);
            roldq(15, k, 4, &mut self.fl_key, 0);
            roldq(30, k, 4, &mut self.subkey, 24);
            roldqo32(34, k, 4, &mut self.subkey, 36);

            roldq(15, ka, 0, &mut self.subkey, 8);
            roldq(30, ka, 0, &mut self.subkey, 20);
            self.fl_key[8] = ka[1];
            self.fl_key[9] = ka[2];
            self.fl_key[10] = ka[3];
            self.fl_key[11] = ka[0];
            roldqo32(49, ka, 0, &mut self.subkey, 40);

            self.subkey[..4].copy_from_slice(kb);
            roldq(30, kb, 0, &mut self.subkey, 12);
            roldq(30, kb, 0, &mut self.subkey, 28);
            roldqo32(51, kb, 0, &mut self.whitening_key, 4);
        } else {
            self.whitening_key[4..].copy_from_slice(&k[..4]);
            decroldqo32(45, k, 0, &mut self.subkey, 28);
            decroldq(15, k, 0, &mut self.fl_key, 4);
            decroldq(17, k, 0, &mut self.subkey, 12);
            decroldqo32(34, k, 0, &mut self.subkey, 0);

            decroldq(15, k, 4, &mut self.subkey, 40);
            decroldq(15, k, 4, &mut self.fl_key, 8);
            decroldq(30, k, 4, &mut self.subkey, 20);
            decroldqo32(34, k, 4, &mut self.subkey, 8);

            decroldq(15, ka, 0, &mut self.subkey, 36);
            decroldq(30, ka, 0, &mut self.subkey, 24);
            self.fl_key[2] = ka[1];
            self.fl_key[3] = ka[2];
            self.fl_key[0] = ka[3];
            self.fl_key[1] = ka[0];
            decroldqo32(49, ka, 0, &mut self.subkey, 4);

            self.subkey[46] = kb[0];
            self.subkey[47] = kb[1];
            self.subkey[44] = kb[2];
            self.subkey[45] = kb[3];
            decroldq(30, kb, 0, &mut self.subkey, 32);
            decroldq(30, kb, 0, &mut self.subkey, 16);
            roldqo32(51, kb, 0, &mut self.whitening_key, 0);
        }
    }
}
...
fn roldq(
    ...
    for index in 0..4 {
        output[output_offset + index] = (input[input_offset + index] << rotation)
            | (input[input_offset + ((index + 1) & 3)] >> (32 - rotation));
    }
    input[input_offset..input_offset + 4]
        .copy_from_slice(&output[output_offset..output_offset + 4]);
}
...
fn decroldq(
    ...
    let rotated = [
        (input[input_offset] << rotation) | (input[input_offset + 1] >> (32 - rotation)),
        (input[input_offset + 1] << rotation) | (input[input_offset + 2] >> (32 - rotation)),
        (input[input_offset + 2] << rotation) | (input[input_offset + 3] >> (32 - rotation)),
        (input[input_offset + 3] << rotation) | (input[input_offset] >> (32 - rotation)),
    ];
    output[output_offset + 2] = rotated[0];
    output[output_offset + 3] = rotated[1];
    output[output_offset] = rotated[2];
    output[output_offset + 1] = rotated[3];
    input[input_offset..input_offset + 4].copy_from_slice(&rotated);
}
...
fn roldqo32(
    ...
    let left = rotation - 32;
    let right = 64 - rotation;
    for index in 0..4 {
        output[output_offset + index] = (input[input_offset + ((index + 1) & 3)] << left)
            | (input[input_offset + ((index + 2) & 3)] >> right);
    }
    input[input_offset..input_offset + 4]
        .copy_from_slice(&output[output_offset..output_offset + 4]);
}
...
fn decroldqo32(
    ...
    let right = 64 - rotation;
    let rotated = [
        (input[input_offset + 1] << left) | (input[input_offset + 2] >> right),
        (input[input_offset + 2] << left) | (input[input_offset + 3] >> right),
        (input[input_offset + 3] << left) | (input[input_offset] >> right),
        (input[input_offset] << left) | (input[input_offset + 1] >> right),
    ];
    output[output_offset + 2] = rotated[0];
    output[output_offset + 3] = rotated[1];
    output[output_offset] = rotated[2];
...
fn camellia_f2(state: &mut [u32; 4], subkey: &[u32], key_offset: usize) {
    let first = state[0] ^ subkey[key_offset];
    let mut u = SBOX4_4404[(first & 0xff) as usize]
        ^ SBOX3_3033[((first >> 8) & 0xff) as usize]
        ^ SBOX2_0222[((first >> 16) & 0xff) as usize]
        ^ SBOX1_1110[(first >> 24) as usize];

    let second = state[1] ^ subkey[key_offset + 1];
    let v = SBOX1_1110[(second & 0xff) as usize]
        ^ SBOX4_4404[((second >> 8) & 0xff) as usize]
        ^ SBOX3_3033[((second >> 16) & 0xff) as usize]
        ^ SBOX2_0222[(second >> 24) as usize];

    state[2] ^= u ^ v;
    state[3] ^= u ^ v ^ u.rotate_right(8);

    let first = state[2] ^ subkey[key_offset + 2];
    u = SBOX4_4404[(first & 0xff) as usize]
        ^ SBOX3_3033[((first >> 8) & 0xff) as usize]
        ^ SBOX2_0222[((first >> 16) & 0xff) as usize]
        ^ SBOX1_1110[(first >> 24) as usize];
    ...
    state[0] ^= u ^ v;
    state[1] ^= u ^ v ^ u.rotate_right(8);
}

fn camellia_fls(state: &mut [u32; 4], fl_key: &[u32], key_offset: usize) {
    state[1] ^= (state[0] & fl_key[key_offset]).rotate_left(1);
    state[0] ^= fl_key[key_offset + 1] | state[1];

    state[2] ^= fl_key[key_offset + 3] | state[3];
    state[3] ^= (fl_key[key_offset + 2] & state[2]).rotate_left(1);
}

impl Zeroize for CamelliaKeySchedule {
    fn zeroize(&mut self) {
        self.subkey.zeroize();
        self.whitening_key.zeroize();
        self.fl_key.zeroize();
        self.key_is_128 = false;
    }
}
