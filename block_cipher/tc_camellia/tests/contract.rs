//! State, buffer and initialization contracts for the Camellia engine.

use tc_block_cipher::{
    BlockCipher, BlockCipherInit, BlockError, CipherDirection, InitError, KeyRef,
};
use tc_camellia_v2::{ALGO_NAME, BLOCK_BYTES, CamelliaEngine, CamelliaLightEngine, KEY_BYTES};
...
#[test]
fn the_camellia_engine_preserves_state_on_errors_and_processes_one_block_through_either_dispatch() {
    for size in KEY_BYTES {
        check_contract(CamelliaEngine::new(), size);
        check_contract(CamelliaLightEngine::new(), size);
    }
}

#[test]
fn a_default_camellia_engine_obeys_the_same_state_and_buffer_contract() {
    for size in KEY_BYTES {
        check_contract(CamelliaEngine::default(), size);
        check_contract(CamelliaLightEngine::default(), size);
    }
}
