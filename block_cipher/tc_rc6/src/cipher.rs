...
/// Constant time with respect to key contents on processors with fixed-latency
/// rotations; key length is public. See the engine's hardware requirements.
pub(crate) fn expand_key(key: &[u8], subkeys: &mut [u32; SUBKEYS]) {
    ...
    for _ in 0..iterations {
        ...
        let mut rotation = a.wrapping_add(b);
        b = words[key_index]
            .wrapping_add(a)
            .wrapping_add(b)
    ...
    a.zeroize();
    b.zeroize();
    words_buffer.zeroize();
}

/// Encrypts one block. Constant time under the engine's hardware requirements.
...
/// Decrypts one block. Constant time under the engine's hardware requirements.
