//! State, buffer and initialization contracts for the Rc6 engine.

use tc_block_cipher::{
    BlockCipher, BlockCipherInit, BlockError, CipherDirection, InitError, KeyRef,
};
use tc_rc6_v2::{ALGO_NAME, BLOCK_BYTES, MAX_KEY_BYTES, Rc6Engine};
        ...
        + 'static,
{
    ...
    for length in [0, MAX_KEY_BYTES + 1] {
        assert_eq!(
            engine.init(
                ...
                &KeyRef::new(&[0; MAX_KEY_BYTES + 1][..length])
            ),
    ...
    for (direction, opposite, input, expected) in [
        ...
        for length in [0, MAX_KEY_BYTES + 1] {
            assert_eq!(
                engine.init(opposite, &KeyRef::new(&[0; MAX_KEY_BYTES + 1][..length])),
                Err(InitError::InvalidKeyLength(length))
            );
...
#[test]
fn the_rc6_engine_preserves_state_on_errors_and_processes_one_block_through_either_dispatch() {
    for size in 1..=MAX_KEY_BYTES {
        check_contract(Rc6Engine::new(), size);
    }
}

#[test]
fn a_default_rc6_engine_obeys_the_same_state_and_buffer_contract() {
    for size in 1..=MAX_KEY_BYTES {
        check_contract(Rc6Engine::default(), size);
    }
}
