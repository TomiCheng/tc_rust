...
pub(crate) trait Word: Copy + Zeroize {
    const BYTES: usize;
    const P: Self;
    const Q: Self;
    const ZERO: Self;
...
impl<W: Word> Core<W> {
    ...
    /// Expands the key. Constant time on processors with operand-independent rotations.
    /// Key length and round count are public; processors without barrel shifters may leak.
    ...
    /// Encrypts one block. Constant time on processors with operand-independent rotations.
    ...
    /// Decrypts one block. Constant time on processors with operand-independent rotations.
...
impl<W: Word> Zeroize for Core<W> {
    fn zeroize(&mut self) {
        self.subkeys.zeroize();
    }
}
