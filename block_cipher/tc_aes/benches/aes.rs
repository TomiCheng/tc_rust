//! AES key setup and single-block benchmarks for all available engines.
//!
//! Run `cargo bench -p tc_aes --bench aes`, or add `--all-features` to
//! include RustCrypto. Optional Criterion filters follow `--`, for example
//! `-- 'aes/encrypt/dispatch-aes-ni/128'`.
//!
...
use tc_aes::{AesEngine, AesLightEngine, AesTableEngine, BLOCK_BYTES, KEY_BYTES};
use tc_block_cipher::{
    BlockCipher, BlockCipherInit, BlockError, CipherDirection, InitError, KeyRef,
};

fn dispatch_name() -> &'static str {
    #[cfg(feature = "rustcrypto")]
    {
        "dispatch-rustcrypto"
    }
...
fn benchmarks(c: &mut Criterion) {
    for (operation, direction, setup) in [
        ...
        let mut group = c.benchmark_group(format!("aes/{operation}"));
        if setup {
        ...
        add_engine(&mut group, "table", direction, setup, AesTableEngine::new);
        add_engine(&mut group, "light", direction, setup, AesLightEngine::new);
        #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
        if AesX86Engine::is_supported() {
            add_engine(&mut group, "aes-ni", direction, setup, || {
                AesX86Engine::new().expect("AES-NI support was checked before benchmarking")
            });
        }
        #[cfg(feature = "rustcrypto")]
        add_engine(
