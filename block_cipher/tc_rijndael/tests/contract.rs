//! State, buffer and initialization contracts for the generalized Rijndael engines.

use tc_block_cipher::{
    BlockCipher, BlockCipherInit, BlockError, CipherDirection, InitError, KeyRef,
};
use tc_rijndael_v2::{ALGO_NAME, KEY_BYTES, RijndaelEngine};
...
fn check_width<const COLUMNS: usize>() {
    for size in KEY_BYTES {
        let invalid = &[0, 15, 17, 19, 21, 23, 25, 27, 29, 31, 33, 64];
        check_contract(
            RijndaelEngine::<COLUMNS>::new(),
            size,
            COLUMNS * 4,
            ALGO_NAME,
            invalid,
        ...
        check_contract(
            RijndaelEngine::<COLUMNS>::default(),
            size,
            COLUMNS * 4,
            ALGO_NAME,
            invalid,
...
#[test]
fn every_rijndael_block_and_key_size_preserves_state_and_buffers_on_errors() {
    check_width::<4>();
    check_width::<5>();
    check_width::<6>();
    check_width::<7>();
