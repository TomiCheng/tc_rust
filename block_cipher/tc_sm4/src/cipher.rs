...
/// Variable time: secret-dependent S-box lookups during key expansion.
pub(crate) fn expand_key(
    for_encryption: bool,
    key: &[u8; KEY_BYTES],
    round_keys: &mut [u32; ROUNDS],
) {
    // Mix the first four words with FK, then advance a sliding window.
    ...
    // Decryption traverses the same round keys in reverse.
    if !for_encryption {
        round_keys.reverse();
    }
    window.zeroize();
...
/// Variable time: secret-dependent S-box lookups during every round.
pub(crate) fn process_block(
    ...
    // Final reversal: output X35, X34, X33, X32.
...
fn t_prime(z: u32) -> u32 {
    let mut b = tau(z);
    let result = b ^ b.rotate_left(13) ^ b.rotate_left(23);
    b.zeroize();
    result
}
...
mod tests {
    ...
    fn the_decryption_schedule_is_the_encryption_schedule_reversed() {
        ...
        let mut encryption = [0; ROUNDS];
        let mut decryption = [0; ROUNDS];
        expand_key(true, &key, &mut encryption);
        expand_key(false, &key, &mut decryption);
        encryption.reverse();
