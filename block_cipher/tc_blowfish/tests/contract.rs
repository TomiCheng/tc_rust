//! State, buffer and initialization contracts for the Blowfish engine.
        ...
        + 'static,
{
    ...
    for length in [0, MIN_KEY_BYTES - 1, MAX_KEY_BYTES + 1] {
        assert_eq!(
            engine.init(
...
#[test]
fn the_blowfish_engine_preserves_state_on_errors_and_processes_one_block_through_either_dispatch() {
    for size in MIN_KEY_BYTES..=MAX_KEY_BYTES {
        check_contract(BlowfishEngine::new(), size);
    }
}
