//! Blowfish constants, key expansion, and block functions.
...
const P_SIZE: usize = ROUNDS + 2;
const S_BOX_SIZE: usize = 256;

const P_INIT: [u32; P_SIZE] = [
    0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344, 0xa4093822, 0x299f31d0, 0x082efa98, 0xec4e6c89,
    0x452821e6, 0x38d01377, 0xbe5466cf, 0x34e90c6c, 0xc0ac29b7, 0xc97c50dd, 0x3f84d5b5, 0xb5470917,
    0x9216d5d9, 0x8979fb1b,
];

const S_INIT: [[u32; S_BOX_SIZE]; 4] = [
    [
...
pub(super) struct BlowfishState {
    p: [u32; P_SIZE],
    s: [[u32; S_BOX_SIZE]; 4],
}

impl BlowfishState {
    /// Creates empty working tables. Constant time: no key is inspected.
    ...
    /// Expands a validated key. Variable time: secret-dependent S-box lookups.
    pub(super) fn expand_key(&mut self, key: &[u8]) {
        self.p = P_INIT;
        self.s = S_INIT;
        ...
        for entry in &mut self.p {
            let mut word = 0u32;
            for _ in 0..4 {
                word = (word << 8) | u32::from(key[key_index]);
                key_index += 1;
        ...
        for index in (0..P_SIZE).step_by(2) {
            block = self.encrypt_words(block.0, block.1);
            self.p[index] = block.0;
            self.p[index + 1] = block.1;
        }

        for box_index in 0..self.s.len() {
            for index in (0..S_BOX_SIZE).step_by(2) {
                block = self.encrypt_words(block.0, block.1);
                self.s[box_index][index] = block.0;
                self.s[box_index][index + 1] = block.1;
            }
        }
        block.0.zeroize();
        block.1.zeroize();
    ...
    /// Encrypts a block. Variable time: secret-dependent S-box lookups.
    ...
    /// Decrypts a block. Variable time: secret-dependent S-box lookups.
    ...
    fn encrypt_words(&self, mut left: u32, mut right: u32) -> (u32, u32) {
        left ^= self.p[0];
        for round in (1..ROUNDS).step_by(2) {
            right ^= self.f(left) ^ self.p[round];
            left ^= self.f(right) ^ self.p[round + 1];
        }
    ...
    fn decrypt_words(&self, mut left: u32, mut right: u32) -> (u32, u32) {
        ...
        for round in (2..=ROUNDS).rev().step_by(2) {
            right ^= self.f(left) ^ self.p[round];
            left ^= self.f(right) ^ self.p[round - 1];
        }
        right ^= self.p[0];
    ...
    fn f(&self, value: u32) -> u32 {
        let result = self.s[0][(value >> 24) as usize]
            .wrapping_add(self.s[1][((value >> 16) & 0xff) as usize])
            ^ self.s[2][((value >> 8) & 0xff) as usize];
        result.wrapping_add(self.s[3][(value & 0xff) as usize])
    }
}

impl Zeroize for BlowfishState {
    fn zeroize(&mut self) {
        self.p.zeroize();
        self.s.zeroize();
    }
