...
/// Supplies an optional tweak to a Threefish engine.
///
/// Timing is implementation-defined; implementations should avoid
/// secret-dependent work when returning the borrowed bytes.
pub trait TweakParams {
    /// Returns a tweak or `None` for the all-zero tweak. Timing is implementation-defined.
...
/// Constant time: construction and access do not inspect key or tweak contents.
#[derive(Clone, Copy)]
pub struct Params<'a> {
    key: &'a [u8],
    tweak: Option<&'a [u8]>,
...
impl<'a> Params<'a> {
    /// Uses `key` with Threefish's all-zero tweak. Constant time.
    pub const fn new(key: &'a [u8]) -> Self {
        Self { key, tweak: None }
    }

    /// Uses `key` with an explicit tweak. Constant time.
...
impl TweakParams for Params<'_> {
    /// Borrows the optional tweak without inspecting it. Constant time.
    fn tweak(&self) -> Option<&[u8]> {
        self.tweak
    }
}
...
impl fmt::Debug for Params<'_> {
    /// Writes public lengths, never key or tweak bytes.
    /// Constant time with respect to secret contents; timing depends on the formatter.
