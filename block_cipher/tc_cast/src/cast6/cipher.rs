...
impl Cast6KeySchedule {
    ...
    /// Variable time: key-dependent S-box indices can leak secret data.
    ...
    /// Variable time: key-dependent S-box indices can leak secret data.
    pub(super) fn encrypt_block(&self, input: &[u8; BLOCK_BYTES], output: &mut [u8; BLOCK_BYTES]) {
    ...
    /// Variable time: key-dependent S-box indices can leak secret data.
    pub(super) fn decrypt_block(&self, input: &[u8; BLOCK_BYTES], output: &mut [u8; BLOCK_BYTES]) {
        let mut state = read_state(input);
...
fn read_state(input: &[u8; BLOCK_BYTES]) -> [u32; 4] {
    core::array::from_fn(|index| {
        u32::from_be_bytes(input[index * 4..index * 4 + 4].try_into().unwrap())
    })
}

fn write_state(state: &[u32; 4], output: &mut [u8; BLOCK_BYTES]) {
    for (index, value) in state.iter().enumerate() {
        output[index * 4..index * 4 + 4].copy_from_slice(&value.to_be_bytes());
    }
}

impl Zeroize for Cast6KeySchedule {
    fn zeroize(&mut self) {
        self.masking.zeroize();
        self.rotations.zeroize();
    }
}
