...
use tc_gost28147_v2::{BLOCK_BYTES, Gost28147Engine, KeyWithSBox, SBoxParams, s_box};
...
fn cfb8_decrypt<P: KeyParams + SBoxParams + ?Sized>(
    params: &P,
    iv: &[u8; BLOCK_BYTES],
    input: &[u8],
) -> Vec<u8> {
    ...
    let mut output = Vec::with_capacity(input.len());
    for &byte in input {
        let mut gamma = [0; BLOCK_BYTES];
        engine.process_block(&feedback, &mut gamma).unwrap();
        output.push(byte ^ gamma[0]);
        feedback.copy_within(1.., 0);
        feedback[BLOCK_BYTES - 1] = byte;
    }
    output
}

#[test]
fn the_default_table_matches_the_block_vector_in_both_directions() {
    let key = unhex(KEY);
    assert_block_vector(
        &KeyWithSBox::new(&key),
        "0000000000000000",
...
#[test]
fn named_tables_match_the_block_vectors_in_both_directions() {
    let key = unhex(KEY);
...
#[test]
fn named_tables_match_the_cfb8_vectors_and_recover_the_plaintext() {
    let key = unhex(KEY);
    ...
    for (table, expected) in [
        ...
        assert_eq!(cfb8_decrypt(&params, &iv, &unhex(expected)), [0; 8]);
    }
}

#[test]
fn a_custom_table_matches_the_cfb8_vector_and_recovers_the_plaintext() {
    let key = unhex(KEY);
    let iv: [u8; BLOCK_BYTES] = unhex("1234567890abcdef").try_into().unwrap();

    // Alternately ascending and descending rows, each still a permutation.
    ...
    assert_eq!(
        cfb8_decrypt(&params, &iv, &unhex("c3af96ef788667c5")),
        [0; 8]
    );
}
