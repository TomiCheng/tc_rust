...
/// Constant time with respect to key contents.
...
/// Transforms a block. Variable time: secret nibbles index the supplied S-box.
pub(crate) fn process_block(
    ...
    // The final round does not swap halves, so both directions share this loop.
...
mod tests {
    ...
    fn the_decryption_schedule_is_the_encryption_schedule_reversed() {
        // Reversing a Feistel network reverses the subkey order.
