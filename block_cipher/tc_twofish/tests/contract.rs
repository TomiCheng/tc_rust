//! State, buffer and initialization contracts for the Twofish engine.

use tc_block_cipher::{
    BlockCipher, BlockCipherInit, BlockError, CipherDirection, InitError, KeyRef,
};
use tc_twofish_v2::{ALGO_NAME, BLOCK_BYTES, KEY_BYTES, TwofishEngine};
...
#[test]
fn the_twofish_engine_preserves_state_on_errors_and_processes_one_block_through_either_dispatch() {
    for size in KEY_BYTES {
        check_contract(TwofishEngine::new(), size);
    }
}

#[test]
fn a_default_twofish_engine_obeys_the_same_state_and_buffer_contract() {
    for size in KEY_BYTES {
        check_contract(TwofishEngine::default(), size);
    }
}
