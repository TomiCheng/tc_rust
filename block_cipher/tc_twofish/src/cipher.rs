//! Twofish key schedule, tables, and block transformations.
...
/// Words in the flattened key-dependent S-box tables (four per byte value).
const SBOX_WORDS: usize = 4 * 256;
...
/// The forty subkeys and the four key-dependent S-box tables, flattened.
pub(crate) struct KeySchedule {
    subkeys: [u32; TOTAL_SUBKEYS],
    sboxes: [u32; SBOX_WORDS],
}

impl KeySchedule {
    /// Creates empty tables. Constant time: no key is inspected.
    pub(crate) const fn new() -> Self {
        Self {
            subkeys: [0; TOTAL_SUBKEYS],
            sboxes: [0; SBOX_WORDS],
        }
    }
}

/// Expands a validated key. Variable time: secret-dependent lookups and branches.
pub(crate) fn expand_key(key: &[u8], schedule: &mut KeySchedule) {
    ...
    let subkeys = &mut schedule.subkeys;
    for i in 0..TOTAL_SUBKEYS / 2 {
        let q = (i as u32).wrapping_mul(SK_STEP);
        let mut a = f32(q, &even, key_words);
        let mut b = f32(q.wrapping_add(SK_BUMP), &odd, key_words).rotate_left(8);
        a = a.wrapping_add(b);
        subkeys[i * 2] = a;
    ...
    let sboxes = &mut schedule.sboxes;
    for i in 0..256 {
        let [mut b0, mut b1, mut b2, mut b3] = keyed_bytes(i as u8, &sbox_keys, key_words);
...
/// Encrypts one block. Variable time: secret-dependent keyed S-box lookups.
...
/// Decrypts one block. Variable time: secret-dependent keyed S-box lookups.
...
fn fe32_0(sboxes: &[u32; SBOX_WORDS], x: u32) -> u32 {
    let b = x.to_le_bytes();
...
fn fe32_3(sboxes: &[u32; SBOX_WORDS], x: u32) -> u32 {
    let b = x.to_le_bytes();
...
fn write_word(output: &mut [u8; BLOCK_BYTES], offset: usize, value: u32) {
...
mod tests {
    ...
    #[test]
    fn the_generated_q_permutations_match_the_known_boundary_values() {
        // Q0/Q1 are generated at compile time; pin their known boundary values.
    ...
    #[test]
    fn the_q_permutations_are_bijective() {
        for table in [&Q0, &Q1] {
    ...
    #[test]
    fn every_supported_twofish_key_length_round_trips() {
        let plaintext = [
        ...
        for key_len in [16, 24, 32] {
            ...
            expand_key(&key[..key_len], &mut schedule);
            let mut ciphertext = [0_u8; BLOCK_BYTES];
            let mut recovered = [0_u8; BLOCK_BYTES];
            encrypt_block(&schedule, &plaintext, &mut ciphertext);
            decrypt_block(&schedule, &ciphertext, &mut recovered);
