...
use crate::{BLOCK_BYTES, KEY_BYTES, XTEA_ALGO_NAME, cipher};

/// XTEA with a 16-byte key and a 8-byte block.
///
...
pub struct XteaEngine {
    working_key: cipher::xtea::Schedule,
    for_encryption: bool,
    initialised: bool,
}
...
impl fmt::Display for XteaEngine {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(XTEA_ALGO_NAME)
    }
}

impl Drop for XteaEngine {
    fn drop(&mut self) {
...
impl BlockCipher for XteaEngine {
    ...
    fn process_block(&mut self, input: &[u8], output: &mut [u8]) -> Result<usize, BlockError> {
        ...
        if self.for_encryption {
            cipher::xtea::encrypt_block(&self.working_key, input, output);
        } else {
            cipher::xtea::decrypt_block(&self.working_key, input, output);
        }
        Ok(BLOCK_BYTES)
    }
}
...
impl<P: KeyParams + ?Sized> BlockCipherInit<P> for XteaEngine {
    ...
    fn init(&mut self, direction: CipherDirection, params: &P) -> Result<(), InitError> {
        ...
        let mut expanded = cipher::xtea::expand_key(key);
