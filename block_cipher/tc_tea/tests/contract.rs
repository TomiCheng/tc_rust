//! State, buffer and initialization contracts for the TEA and XTEA engines.

use tc_block_cipher::{
    BlockCipher, BlockCipherInit, BlockError, CipherDirection, InitError, KeyRef,
};
use tc_tea_v2::{BLOCK_BYTES, KEY_BYTES, TEA_ALGO_NAME, TeaEngine, XTEA_ALGO_NAME, XteaEngine};

fn check_contract<E>(mut engine: E, name: &str)
where
    E: core::fmt::Display
...
#[test]
fn both_tea_engines_preserve_state_on_errors_and_process_one_block_through_either_dispatch() {
    check_contract(TeaEngine::new(), TEA_ALGO_NAME);
    check_contract(XteaEngine::new(), XTEA_ALGO_NAME);
}

#[test]
fn both_default_tea_engines_obey_the_same_state_and_buffer_contract() {
    check_contract(TeaEngine::default(), TEA_ALGO_NAME);
    check_contract(XteaEngine::default(), XTEA_ALGO_NAME);
}
