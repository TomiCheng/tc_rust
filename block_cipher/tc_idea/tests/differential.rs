//! IDEA agrees with RustCrypto after establishing the KAT byte order.

mod common;

use common::unhex;
use idea::Idea;
use idea::cipher::{Block, BlockCipherDecrypt, BlockCipherEncrypt, KeyInit};
use tc_block_cipher::{BlockCipher, BlockCipherInit, CipherDirection, KeyRef};
use tc_idea_v2::{BLOCK_BYTES, IdeaEngine, KEY_BYTES};

#[test]
fn idea_matches_rustcrypto_on_the_known_vector_and_64_key_block_pairs() {
    let key = unhex("00010002000300040005000600070008");
    let plaintext = unhex("0000000100020003");
    let ciphertext = unhex("11FBED2B01986DE5");
    let reference = Idea::new_from_slice(&key).unwrap();
    let mut block = Block::<Idea>::try_from(plaintext.as_slice()).unwrap();
    ...
    for _ in 0..64 {
        ...
        for direction in [CipherDirection::Encrypt, CipherDirection::Decrypt] {
            let mut expected = Block::<Idea>::from(input);
            match direction {
