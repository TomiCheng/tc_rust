//! State, buffer and initialization contracts for the SEED engine.

use tc_block_cipher::{
    BlockCipher, BlockCipherInit, BlockError, CipherDirection, InitError, KeyRef,
};
use tc_seed_v2::{ALGO_NAME, BLOCK_BYTES, KEY_BYTES, SeedEngine};
...
#[test]
fn the_seed_engine_preserves_state_on_errors_and_processes_one_block_through_either_dispatch() {
    check_contract(SeedEngine::new());
}

#[test]
fn a_default_seed_engine_obeys_the_same_state_and_buffer_contract() {
    check_contract(SeedEngine::default());
}
