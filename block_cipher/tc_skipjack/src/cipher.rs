...
/// Constant time with respect to key contents: all indices are public.
pub(crate) fn expand_key(key: &[u8; KEY_BYTES], schedule: &mut Schedule) {
...
/// Encrypts one block. Variable time: secret-dependent F-table lookups.
...
/// Decrypts one block. Variable time: secret-dependent F-table lookups.
pub(crate) fn decrypt_block(
    schedule: &Schedule,
    input: &[u8; BLOCK_BYTES],
    output: &mut [u8; BLOCK_BYTES],
) {
    // Swap word roles and reverse the steps, replacing g with its inverse h.
...
mod tests {
    ...
    #[test]
    fn the_inverse_permutation_recovers_every_possible_word() {
    ...
    fn the_schedule_cycles_the_key_bytes() {
        ...
        let mut schedule = [[0; STEP_KEY_BYTES]; STEPS];
        expand_key(&key, &mut schedule);
        assert_eq!(schedule[0], [0, 1, 2, 3]);
        assert_eq!(schedule[1], [4, 5, 6, 7]);
        // The third step wraps past the key's end to its beginning.
