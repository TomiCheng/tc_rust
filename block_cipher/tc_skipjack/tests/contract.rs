//! State, buffer and initialization contracts for the SKIPJACK engine.

use tc_block_cipher::{
    BlockCipher, BlockCipherInit, BlockError, CipherDirection, InitError, KeyRef,
};
use tc_skipjack_v2::{ALGO_NAME, BLOCK_BYTES, KEY_BYTES, SkipjackEngine};
        ...
        + 'static,
{
    ...
    for length in [0, KEY_BYTES - 1, KEY_BYTES + 1, 16, 24, 32] {
        assert_eq!(
...
#[test]
fn the_skipjack_engine_preserves_state_on_errors_and_processes_one_block_through_either_dispatch() {
    check_contract(SkipjackEngine::new());
}

#[test]
fn a_default_skipjack_engine_obeys_the_same_state_and_buffer_contract() {
    check_contract(SkipjackEngine::default());
}
