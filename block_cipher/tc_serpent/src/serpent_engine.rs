...
use crate::{BLOCK_BYTES, KEY_STEP_BYTES, MAX_KEY_BYTES, MIN_KEY_BYTES, SERPENT_ALGO_NAME, cipher};

/// Serpent with a 4- to 32-byte key in four-byte steps and a 16-byte block.
/// Uses the conventional little-endian representation.
///
...
/// use tc_serpent_v2::{BLOCK_BYTES, MAX_KEY_BYTES, SerpentEngine};
///
...
impl fmt::Display for SerpentEngine {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(SERPENT_ALGO_NAME)
    }
}

impl Drop for SerpentEngine {
    fn drop(&mut self) {
...
impl BlockCipher for SerpentEngine {
    ...
    fn process_block(&mut self, input: &[u8], output: &mut [u8]) -> Result<usize, BlockError> {
        ...
        if self.for_encryption {
            cipher::encrypt_block(&self.working_key, Representation::Serpent, input, output);
        } else {
            cipher::decrypt_block(&self.working_key, Representation::Serpent, input, output);
        }
        Ok(BLOCK_BYTES)
    }
}
...
impl<P: KeyParams + ?Sized> BlockCipherInit<P> for SerpentEngine {
    ...
    fn init(&mut self, direction: CipherDirection, params: &P) -> Result<(), InitError> {
        ...
        let mut expanded = cipher::expand_key(key, Representation::Serpent);
