...
pub(crate) const WORKING_KEY_WORDS: usize = (ROUNDS + 1) * 4;
...
/// Expands a validated key. Constant time with respect to key contents.
pub(crate) fn expand_key(key: &[u8], representation: Representation) -> [u32; WORKING_KEY_WORDS] {
...
fn encrypt_state(mut state: [u32; 4], working_key: &[u32; WORKING_KEY_WORDS]) -> [u32; 4] {
...
fn decrypt_state(mut state: [u32; 4], working_key: &[u32; WORKING_KEY_WORDS]) -> [u32; 4] {
    for i in 0..4 {
        state[i] ^= working_key[ROUNDS * 4 + i];
    }
...
/// Encrypts one block. Constant time with respect to key and block contents.
pub(crate) fn encrypt_block(
    ...
    let state = encrypt_state(read_state(input, representation), working_key);
    write_state(state, representation, output);
}

/// Decrypts one block. Constant time with respect to key and block contents.
pub(crate) fn decrypt_block(
    ...
    let state = decrypt_state(read_state(input, representation), working_key);
    write_state(state, representation, output);
}

/// Loads the block into the four state words.
///
/// Serpent reads little-endian in order; Tnepres reads big-endian with both the
/// word order and the bytes within each word reversed. That difference is the
/// whole of what separates the two engines.
fn read_state(input: &[u8; BLOCK_BYTES], representation: Representation) -> [u32; 4] {
    ...
    match representation {
        ...
        Representation::Tnepres => {
            for (word, chunk) in state.iter_mut().zip(input.rchunks_exact(4)) {
                *word = u32::from_be_bytes(chunk.try_into().unwrap());
            }
        }
    }
...
/// Stores the state words back into a block, mirroring [`read_state`].
fn write_state(state: [u32; 4], representation: Representation, output: &mut [u8; BLOCK_BYTES]) {
    match representation {
        ...
        Representation::Tnepres => {
            for (word, chunk) in state.iter().zip(output.rchunks_exact_mut(4)) {
...
mod tests {
    ...
    #[test]
    fn both_representations_round_trip() {
        let key = [0xa5_u8; 32];
        let plaintext = [
        ...
        for representation in [Representation::Serpent, Representation::Tnepres] {
            let working_key = expand_key(&key, representation);
            let mut ciphertext = [0_u8; BLOCK_BYTES];
            let mut recovered = [0_u8; BLOCK_BYTES];
            encrypt_block(&working_key, representation, &plaintext, &mut ciphertext);
            decrypt_block(&working_key, representation, &ciphertext, &mut recovered);
    ...
    #[test]
    fn the_two_representations_disagree_on_the_same_bytes() {
        // Tnepres is not an alias: the same bytes produce different ciphertext.
        let key = [0x5a_u8; 16];
        let plaintext = [0x01_u8; BLOCK_BYTES];
        let mut serpent = [0_u8; BLOCK_BYTES];
        let mut tnepres = [0_u8; BLOCK_BYTES];

        let representation = Representation::Serpent;
        encrypt_block(
            &expand_key(&key, representation),
            representation,
            &plaintext,
            &mut serpent,
        );
        let representation = Representation::Tnepres;
        encrypt_block(
            &expand_key(&key, representation),
            representation,
            &plaintext,
