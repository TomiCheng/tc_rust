...
/// Constant time with respect to key contents; direction is public.
pub(crate) fn prepare_key(for_encryption: bool, key: &[u8; KEY_BYTES], working_key: &mut [u32; 4]) {
    read_words(key, working_key);
    if !for_encryption {
        theta(working_key, &[0; 4]);
    }
}
...
pub(crate) fn encrypt_block(
    ...
    // Output transform: the last constant and theta, without a nonlinear layer.
...
pub(crate) fn decrypt_block(
    ...
    // Reverse round order: theta precedes the constant, consumed in reverse.
...
fn read_words(input: &[u8; BLOCK_BYTES], words: &mut [u32; 4]) {
...
mod tests {
    ...
    #[test]
    fn the_second_bit_shuffle_undoes_the_first() {
        let mut state = STATE;
        pi1(&mut state);
        assert_ne!(state, STATE);
        pi2(&mut state);
    ...
    fn theta_with_a_zero_key_is_an_involution() {
        // The decryption key uses this property: theta's linear part is involutive.
        let mut state = STATE;
        theta(&mut state, &[0; 4]);
        assert_ne!(state, STATE);
        theta(&mut state, &[0; 4]);
