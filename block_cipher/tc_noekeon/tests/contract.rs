//! State, buffer and initialization contracts for the Noekeon engine.

use tc_block_cipher::{
    BlockCipher, BlockCipherInit, BlockError, CipherDirection, InitError, KeyRef,
};
use tc_noekeon_v2::{ALGO_NAME, BLOCK_BYTES, KEY_BYTES, NoekeonEngine};
...
#[test]
fn the_engine_preserves_state_on_errors_and_processes_one_block_through_either_dispatch() {
    check_contract(NoekeonEngine::new());
}

#[test]
fn default_constructed_engines_obey_the_same_contract() {
    check_contract(NoekeonEngine::default());
}
