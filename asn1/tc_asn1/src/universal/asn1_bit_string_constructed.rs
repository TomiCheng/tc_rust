...
const SEGMENT_RULES: EncodingOptions = EncodingOptions::new(EncodingType::Ber(LengthForm::Definite));

/// CER segments carry 1000 contents octets (X.690 §9.2).
pub(crate) const CER_SEGMENT_LEN: usize = 1000;

/// The identifier with the constructed bit cleared, for the primitive fallbacks.
fn primitive_tag(tag: &[u8]) -> Vec<u8> {
